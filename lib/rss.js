...
export async function getStoredArticles() {
  try {
    // Try to get from Netlify Blobs first (works in production)
    try {
      ...
      // Ignore blob error, likely running locally without Netlify CLI
    }

    // Fallback to local file for development or first run
...
export async function saveArticles(articles) {
  try {
    // Try to save to Netlify Blobs (works in production)
    try {
      const store = getStore('articles');
      await store.setJSON('feed', articles);
    } catch (blobErr) {
      // Ignore blob error locally
    }

    // Attempt to save locally (fails silently in production Lambda but works in dev)
