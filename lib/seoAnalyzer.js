...
// ── 2. Meta Description (15pts) (Kept original to maintain 100 points) ───────
...
function scoreStructure(rawHtml, wordCount) {
  ...
  if (h2s >= 2) { 
    score += 10; notes.push(`✓ Good use of ${h2s} H2 tags for hierarchy`); 
  } else if (h2s === 1) {
    score += 5; notes.push('⚠ Only one H2 tag found — add more for better scannability');
  } else { 
    notes.push('✗ Missing H2 structure; use subheadings for scannability'); 
  }

  // Expertise Signals (E-E-A-T)
  // Checking for citations or "Experience" language
  if (/\b(research|study|according to|source|expert|data)\b/i.test(contentToSearch)) {
    score += 10; notes.push('✓ Evidence-based language detected (E-E-A-T signal)');
  } else {
    notes.push('⚠ No E-E-A-T signals detected — add sources or data references');
  }

  // Depth vs Intent
  if (wordCount >= 1000) { 
    score += 5; notes.push(`✓ Comprehensive depth (${wordCount} words) for informative intent`); 
